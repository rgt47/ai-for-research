...
# Violet gradient anchored on the brand color #6a4c93,
...
# Vertical gradient from deep violet to pale paper.
ramp <- grDevices::colorRampPalette(
  c("#2a1f3d", brand, "#9c85bd", "#cabbe0", "#f2eef7")
)
...
draw_cover <- function(path, width, height, with_text) {
  ...
  # Faint stratigraphy suggesting document layers.
  ...
  if (with_text) {
    graphics::text(0.5, 0.62, "Leveraging AI Tools",
      col = "#f7f4fa", cex = 2.5, font = 2)
    graphics::text(0.5, 0.53, "for Academic Research",
      col = "#f7f4fa", cex = 2.5, font = 2)
    graphics::text(0.5, 0.41, "A One-Week Boot Camp",
      col = "#f7f4fa", cex = 1.5, font = 3)
    graphics::text(0.5, 0.08, "The rgtlab Curriculum Project",
      col = grDevices::adjustcolor("#f7f4fa", 0.85),
      cex = 1.0)
  }
}
